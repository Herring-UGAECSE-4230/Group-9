...
# GPIO pins connected to the 'X' lines will be set up as inputs to the pad/output from the PI
GPIO.setup(ROW_1, GPIO.IN)
GPIO.setup(ROW_2, GPIO.IN)
GPIO.setup(ROW_3, GPIO.IN)
GPIO.setup(ROW_4, GPIO.IN)

# Pins connected to the 'Y' lines will be set up as outputs from the pad/inputs to the PI
# If needed set low by default: pull_up_down=GPIO.PUD_DOWN
GPIO.setup(COL_1, GPIO.OUT)
GPIO.setup(COL_2, GPIO.OUT)
GPIO.setup(COL_3, GPIO.OUT)
...
# Physical keyboard layout
# Loop checking each row
print("Press buttons on the keypad. Ctrl+C to exit.")
try:
    while True:
        key_pressed = readKeypad(ROW_1, [1, 2, 3, 'A'])
        if key_pressed is not None:
            print(f"Key pressed: {key_pressed}")
        
        key_pressed = readKeypad(ROW_2, [4, 5, 6, 'B'])
        if key_pressed is not None:
            print(f"Key pressed: {key_pressed}")
        
        key_pressed = readKeypad(ROW_3, [7, 8, 9, 'C'])
        if key_pressed is not None:
            print(f"Key pressed: {key_pressed}")
        
        key_pressed = readKeypad(ROW_4, ['*', 0, '#', 'D'])
        if key_pressed is not None:
            print(f"Key pressed: {key_pressed}")
